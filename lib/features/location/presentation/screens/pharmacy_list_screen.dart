...
class PharmacyListScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState createState() => _PharmacyListScreenState();
}
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              SizedBox(
                height: size.height * 542 / 852,
                child: (selectedTab == 0)
                    ? ref.watch(getPracticesProvider).when(
                        data: (practices) {
                          if (practices.length == 0) return Container();
                          return ListView.builder(
                              itemBuilder: (context, index) {
                            final practice = practices[index];
                            return PlacesListTile(practice: practice);
                          });
                        },
                        error: (err, st) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    ...
                    : ref.watch(getFavPracticesProvider(user!)).when(
                        data: (practices) {
                          return ListView.builder(
                              itemBuilder: (context, index) {
                            final practice = practices[index];
