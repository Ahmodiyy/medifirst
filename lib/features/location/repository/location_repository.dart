...
class LocationRepository {
  ...
  Stream<List<DoctorInfo>> getFavoriteDoctors(UserInfoModel user) {
    return _doctors.where('doctorId', whereIn: user.favDoctors).snapshots().map((event) {
      return event.docs
          .map(
            (e) => DoctorInfo.fromMap(
          e.data() as Map<String, dynamic>,
  ...
  Stream<List<HealthcareCentreInfo>> getFavoritePractices(UserInfoModel user) {
    return _practices.where('pId', whereIn: user.favDoctors).snapshots().map((event) {
      return event.docs
          .map(
            (e) => HealthcareCentreInfo.fromMap(
          e.data() as Map<String, dynamic>,
  ...
  FutureVoid uploadPrescriptionsToPharmacy(HealthcareCentreInfo pharmacy, List<PrescriptionInfo> prescriptions) async{
    try{
