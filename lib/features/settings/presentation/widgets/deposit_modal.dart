                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border:
                              Border.all(color: Palette.dividerGray, width: 1),
                        ),
                        child: Center(
                ...
                onTap: () async {
                  if (_controller.text.isNotEmpty) {
                    await PayWithPayStack().now(
                      context: context,
                      secretKey: Secret.paystackSecretTest,
                      customerEmail: user!.email,
                      reference: const Uuid().v4(),
                      callbackUrl: 'https://medifirsttechnology.com/',
                      currency: 'NGN',
                      paymentChannel: ['bank_transfer', 'card', 'mobile_money', 'bank'],
                      amount: int.parse(_controller.text.trim()).toDouble(),
                      transactionCompleted: (paymentDate)async{
                        await ref.read(transactionsControllerProvider).logDeposit(context, double.parse(_controller.text.trim()), user.uid);
                      },
                      transactionNotCompleted: (paymentData) {
