...
class _ExplorePageState extends ConsumerState<ExplorePage> {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: ExploreAppBar(uid: user?.uid ?? '',  name: user?.name ?? '', profilePic: user?.profilePicture ?? ''),
                  ),
  ...
  Widget _buildSearchBar(BuildContext context, Size size) {
    return Padding(
      ...
      child: InkWell(
        onTap: () async {
          Navigator.push(context, MaterialPageRoute(builder: (context) => const DoctorSearchPage()));
  ...
  Widget _buildMedicationBar(WidgetRef ref, String uid) {
    return ref.watch(nextMedicationProvider(uid)).when(
      ...
      error: (error, stackTrace) => const ErrorText(error: 'No medications scheduled'),
      loading: () => const Loader(),
    );
  }
