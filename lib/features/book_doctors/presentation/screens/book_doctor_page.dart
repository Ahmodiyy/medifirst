...
class _BookDoctorPageState extends ConsumerState<BookDoctorPage> {
  ...
  Future<void> setAppointment(WidgetRef ref, BuildContext context, UserInfoModel user)async{
    try{
      setState(() {
        loading = true;
      });
      ...
      if(balance >= widget.doctorInfo.consultationFee) {
        ...
        DateTime? remindTime = selectedDate?.subtract(Duration(minutes: _selectedTime));
      ...
      showModalBottomSheet(context: context, builder: (context)=> ErrorModal(message: e.toString()));
          ...
          child: Column(
            children: [
              Container(
                ...
                padding:
                EdgeInsets.symmetric(vertical: size.height * 20/852, horizontal: size.width * 16/393),
              ...
              SetTimeSection(
                ...
                child: InkWell(
                  onTap: () async {
                    TimeOfDay? getTime = await showTimePicker(context: context, initialTime: TimeOfDay.fromDateTime(selectedDate!));
                    if(getTime != null){
                      selectedDate = DateTime(selectedDate!.year, selectedDate!.month, selectedDate!.day, getTime.hour, getTime.minute);
                      setState(() {
              ...
              (size.height * 24/852).pv,
              const SectionHeadingText(heading: 'REMINDER').sidePad(16).alignLeft(),
              (size.height * 12/852).pv,
              SetTimeSection(
                label: 'Set reminder',
              ...
              InkWell(
                  ...
                  child: loading? const CircularProgressIndicator(color: Palette.mainGreen,) : const ActionButtonContainer(
                      title: 'Book an appointment')).sidePad(size.width * 16/393),
              (size.height * 24/852).pv,
            ],
          ),
        ),
