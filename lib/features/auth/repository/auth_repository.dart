...
import 'package:medifirst/features/auth/presentation/screens/phone_verification_screen.dart';
...
final authRepoProvider = Provider((ref) {
  ...
  return AuthRepository(
      auth: auth, firestore: firestore, googleSignIn: google, ref: ref, storageRepo: storageRepo);
});
...
class AuthRepository {
  ...
  CollectionReference get _doctors => _firestore.collection(FirebaseConstants.doctorsCollection);
  ...
  Future<DoctorInfo> getDoctorDetails() async {
    User currentUser = _auth.currentUser!;
    DocumentSnapshot snapshot =
    await _doctors.doc(currentUser.uid).get();
  ...
  Future<HealthcareCentreInfo> getPharmacyDetails()async{
    User currentUser = _auth.currentUser!;
    DocumentSnapshot snapshot =
    await _practices.doc(currentUser.uid).get();
    return HealthcareCentreInfo.fromMap(snapshot.data() as Map<String, dynamic>);
  }
  ...
  Future<DoctorInfo> getDoctorInfo(String uid) async {
    DocumentSnapshot snapshot =
    await _doctors.doc(uid).get();
    return DoctorInfo.fromMap(snapshot.data() as Map<String, dynamic>);
  }

  Future<HealthcareCentreInfo> getPharmacyInfo(String uid)async{
    DocumentSnapshot snapshot =
    await _practices.doc(uid).get();
    return HealthcareCentreInfo.fromMap(snapshot.data() as Map<String, dynamic>);
  }

  Stream<model.UserInfoModel> getUserData(String uid) {
    return _users.doc(uid).snapshots().map(
            (event) => model.UserInfoModel.fromMap(event.data() as Map<String, dynamic>));
  }
  ...
  FutureEither<String> signInWithGoogle() async {
    try {
      ...
      if (userCred.additionalUserInfo!.isNewUser) {
        ...
        switch (category){
          case Constants.patientCategory:
            final userModel = model.UserInfoModel(name: userCred.user!.displayName ?? 'No name', surname: '', uid: userCred.user!.uid, profilePicture: userCred.user!.photoURL ?? Constants.defaultProfilePic, email: userCred.user!.email ?? '', phone: userCred.user!.phoneNumber ?? '', favDoctors: [], favPractices: []);
          ...
          case Constants.doctorCategory:
            ...
            break;
          case Constants.pharmacyCategory:
            final pharmacy = HealthcareCentreInfo(latitude: 0, longitude: 0, pId: userCred.user!.uid, name: userCred.user!.displayName ?? 'No name', address: '', pharmacyImgUrl: userCred.user!.photoURL ?? Constants.defaultProfilePic, number: userCred.user!.phoneNumber ?? '', emergencyContact: '', state: 'Lagos', lga: 'Shomolu', type: '', openingHours: const TimeOfDay(hour: 9, minute: 0), closingHours: const TimeOfDay(hour: 17, minute: 0), totalRating: 0, noOfReviews: 0);
            await _practices.doc(pharmacy.pId).set(pharmacy.toMap());
            break;
        }
        final WalletInfo wallet = WalletInfo(uid: userCred.user!.uid, balance: 0);
  ...
  FutureVoid registerUserWithNumber(String mobile, String name, String surname, BuildContext context) async {
    ...
    try {
      await _auth.verifyPhoneNumber(
        ...
        verificationCompleted: (PhoneAuthCredential credential) async {
          try {
            ...
            print("Auto-verification completed for user: ${userCred.user?.uid}");
        ...
        codeSent: (String verificationId, int? forceResendingToken) {
          ...
          // Don't complete the completer here, as we're waiting for user input
        },
      ...
      print("Error in registerUserWithNumber: $e");
      throw e;
    }
  }

  FutureVoid loginUserWithNumber (String mobile, BuildContext context) async {
    try {
      DocumentSnapshot name = await _numbers.doc(mobile).get();
      if(name.data() == null){
        throw Exception('Number has not be registered');
      }
      final json = name.data() as Map<String, dynamic>;
      ...
      if(number != null){
        ...
        throw Exception('Number does not exist');
  ...
  FutureVoid loginUserWithEmail({required String email, required String password,})async{
    try{
  ...
  FutureEither<String> verifyOTPFromSignUp({
    ...
    try {
      ...
      final res = await saveUserInDatabase(uid: uid, name: name, surname: surname, phone: number);
  ...
  FutureEither<String> verifyOTPFromLogin({
    required BuildContext context,
    required String verificationId,
    required String OTP,
    required String number,
  ...
  FutureEither<String> saveUserInDatabase(
    ...
    try {
      ...
      switch (type){
        case Constants.patientCategory:
          model.UserInfoModel user =
          model.UserInfoModel(name: name, surname: surname, uid: uid, phone: phone, email: email, state: 'Lagos', city: 'Shomolu', profilePicture: profilePic, favDoctors: [], favPractices: []);
        ...
        case Constants.doctorCategory:
          ...
          break;
        case Constants.pharmacyCategory:
          HealthcareCentreInfo pharmacy = HealthcareCentreInfo(latitude: 0, longitude: 0, pId: uid, name: name, pharmacyImgUrl: profilePic, address: '', number: phone, type: '', openingHours: const TimeOfDay(hour: 9, minute: 0), closingHours: const TimeOfDay(hour: 17, minute: 0), totalRating: 0, noOfReviews: 0, emergencyContact: '', state: '', lga: '');
          await _practices.doc(pharmacy.pId).set(pharmacy.toMap());
          break;
      }
