...
class FirebaseService {
  ...
  Future<List<String>> fetchSpecialties() async {
    try {
      QuerySnapshot querySnapshot = await _firestore.collection('specialties').get();
    ...
    try {
      ...
      if(mounted){
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                const EditDoctorProfileScreenTwo()));
      }
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: ref.watch(doctorSettingsInfoProvider(doctor!.doctorId)).when(
          data: (doctorInfo) {
            ...
            _ageController.text = (doctorInfo.age != 0)? doctorInfo.age.toString() : '';
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SectionHeadingText(heading: 'SET UP PROFILE').alignLeft(),
                        Row(
                          children: [
                            SizedBox(
                        ...
                        InkWell(
                          onTap: () async {
                            await saveDetailsAndContinue(ref: ref, context: context, doc: doctorInfo, ogImg: doctorInfo.doctorImage);

                        },
                          child:  isLoading? const CircularProgressIndicator(color: Palette.mainGreen,): const ActionButtonContainer(title: 'Continue'),
                        ),
                      ],
                    ),
                  ),
