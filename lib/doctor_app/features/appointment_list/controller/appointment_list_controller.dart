...
final appointmentListControllerProvider = Provider<AppointmentListController>((ref) {
  final repo = ref.watch(appointmentListRepoProvider);
  return AppointmentListController(repo: repo);
});

final getAllDoctorAppointmentsProvider = StreamProvider.family((ref, String doctorId) {
  return ref.watch(appointmentListControllerProvider).getAllDoctorAppointments(doctorId);
});

final getDoctorAppointmentRequestsProvider = StreamProvider.family((ref, String doctorId) {
  return ref.watch(appointmentListControllerProvider).getDoctorAppointmentRequests(doctorId);
});

final getDoctorVideoAppointmentsProvider = StreamProvider.family((ref, String doctorId) {
  return ref.watch(appointmentListControllerProvider).getDoctorVideoAppointments(doctorId);
});

final getDoctorVoiceAppointmentsProvider = StreamProvider.family((ref, String doctorId) {
  return ref.watch(appointmentListControllerProvider).getDoctorVoiceAppointments(doctorId);
});

final getDoctorChatAppointmentsProvider = StreamProvider.family((ref, String doctorId) {
  return ref.watch(appointmentListControllerProvider).getDoctorChatAppointments(doctorId);
});

class AppointmentListController{
  final AppointmentListRepository _repo;
  const AppointmentListController({required AppointmentListRepository repo}) : _repo = repo;
